...
def make_function(*, function, name, arity, wrap=True, size =10):
    ...
    # Check output shape
    args = [np.ones(size) for _ in range(arity)]
    try:
        function(*args)
    ...
    if wrap:
        return _Function(function=wrap_non_picklable_objects(function),
                         name=name,
                         arity=arity)
    return_func = _Function(function=function,
                     name=name,
                     arity=arity)
